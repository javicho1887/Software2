...
function RecuperarContraseñaDocente() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/validar-correo-docente/', { // Incluye el prefijo 'api/'
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        setTimeout(() => {
          setShowPopup(false);
          navigate('/nueva-contraseña-docente', { state: { email: emailTrimmed } });
        }, 2000);
      } else {
...
export default RecuperarContraseñaDocente;
